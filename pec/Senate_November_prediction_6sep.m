...
systematic=0.7; % Guess at systematic error this year
...
sd_mm=sqrt(sd_mm+Orman_offset*Orman_offset); % stuff happens between now and November
...
if and(h<=35,h>0) % election is soon, so combine current and long-term
    ...
    predicted_mm=(mean_mm*h+current_mm*(35-h))/35;
...
dlmwrite('Senate_D_November_control_probability.csv',[today-datenum('31-Dec-2013') D_November_control_probability])
